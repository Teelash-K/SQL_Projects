...
SELECT Products.Product_Name, MAX(Orders.Order_Quantity) AS Total_Quantity
FROM Products 
FULL OUTER JOIN Orders  ON Products.Product_ID = Orders.Product_ID
GROUP BY Products.Product_Name
...
-- SELECT Customers.Customer_Name, Orders.Order_Date, COUNT(Orders.Order_Quantity) AS Total_Quantity
-- FROM Customers
-- FULL OUTER JOIN Orders NO Customers.Customer_ID = Orders.Customer_ID
-- GROUP BY Customers.Customer_Name
-- HAVING COUNT()
-- ORDER BY Total_Quantity 

SELECT Customers.Customer_Name, COUNT(Orders.Order_Date) AS Total_Orders
FROM Customers
JOIN Orders ON Customers.Customer_ID = Orders.Customer_ID
GROUP BY Customers.Customer_Name
HAVING COUNT(DISTINCT DATEPART(WEEKDAY, Orders.Order_Date)) = 2;   

SELECT Customers.Customer_Name, COUNT(DISTINCT Orders.Order_Date) AS Total_Weekdays
FROM Customers 
FULL OUTER JOIN Orders ON Customers.Customer_ID = Orders.Customer_ID
GROUP BY Customers.Customer_Name, Orders.Order_Date
HAVING COUNT(DISTINCT Orders.Order_Date) = 2;

--  no customer place an order on every day of the week
